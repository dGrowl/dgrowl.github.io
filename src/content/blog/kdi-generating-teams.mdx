...
There's an efficient algorithm available to randomly sort players into teams--just shuffle a list of names, then split it into subarrays of length `max_size`.
...
However, this approach lacks a number of characteristics that kdi needs. Most importantly, we want players to have **different teammates each round**. We could repeatedly shuffle until a sufficiently scrambled teamset is found, but that would take quite a while after a few rounds. It would also be nice if our algorithm facilitated leadership via **captains spread into different teams**. Finally, if `max_size` doesn't cleanly divide `len(players)`, one team could be much smaller than the others. We'd prefer if **teams were sized as fairly as possible**. Let's figure out a better way that accomplishes these goals.
...
<Figure i={2}>
	...
	/>
	<span>
		A sample team graph in which **Ana** has played with **Bea** twice and
		**Cal** once.
	</span>
</Figure>
...
<Figure i={4}>
	...
	<span>
		A sample graph and suboptimal teamset resulting from the collapse of
		$\overline{BD}$ followed by $\overline{AC}$.
	</span>
</Figure>
...
<Figure i={5}>
	...
	<span>
		...
		resulting from the guaranteed collapses of $\overline{AD}$ and $\overline
		{BC}$.
	</span>
</Figure>

Revisiting our four player sample graph, $w(A, B) = 1$ and $w(A, C) = 1$ are subtracted from the other edges that involve their nodes. Now when we generate teams, the result is optimal; none of the players on these teams have with each other before. They're happy, and we're happy!

### Team Leaders

Next, we'll tackle the idea of "captains": players that lead the teams, typically community moderators. Captains should be separated into different teams _unless_ there are no eligible non-captains remaining to pair. We can add a large constant e.g. 100 to the weights of edges between captains. This will make our algorithm avoid collapsing those edges unless it's the only option.
...
<Figure i={6}>
	...
	/>
	<span>A teams graph in which **Ana** and **Cal** are captains.</span>
</Figure>
...
<Figure i={7}>
	...
	/>
	<span>
		An updated version of the teams graph from Figure 6, with edge weights
		reflecting the captains' influence.
	</span>
</Figure>
...
We've covered a lot of ground, iterating upon our simple design to achieve the features we desired. The actual process of finding this algorithmic approach took a lot of experimentation and refinement. I was driven by the needs of the community and it's been very rewarding to see the fruits of my work--especially in cases where the simple approach would have failed my users. Using kdi regularly (see [dogfooding](https://en.wikipedia.org/wiki/Eating_your_own_dog_food)) has been very helpful in the development process, from finding inspiration for improvements to discovering embarrassing bugs _in situ_. kdi is one of my favorite projects and I hope to continue working on it for a long time.
